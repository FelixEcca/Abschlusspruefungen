  ...
  intro({ data }) {
    return (
      <p>
        ...
        <InlineMath
          math={`y=${data.a != 1 ? pp(data.a) : ''}x²${-2 * data.a * data.h != 0 ? pp(-2 * data.a * data.h, 'merge_op') + 'x' : ''}${pp(
        ...
        return (
          <>
            {-2 * data.a * data.h != 0 ? (
              <>
                <p>
                  Bestimme die Scheitelpunktform mit quadratischer Ergänzung:
                </p>
                <BlockMath
                ...
                />
                <BlockMath
                  math={`S\\,=\\,(${pp(data.h)}\\mid ${pp(data.k)})`}
                />
              </>
            ) : (
              <>
                <p>
                  Die Parabel p liegt mit dem Scheitel auf der y-Achse:<br></br>{' '}
                  <BlockMath math={`S\\,=\\,(0\\mid 1)`} />
                </p>
              </>
            )}
          </>
          ...
          <>
            {data.h === 0 && (
              <>
                ...
                <BlockMath math={`2x²-1=-x+2`} />
                <BlockMath math={`2x²+x-3=0`} />
                <BlockMath math={`x²+0,5x-1,5=0`} />
                <p>Löse mithilfe der pq-Formel: </p>
                <BlockMath
                  math={`x_{1,2}=-\\frac{0,5}{2}\\pm\\sqrt{\\left(\\frac{0,5}{2}\\right)^2+1,5}`}
                />
                <BlockMath math={`x_{1,2}=-0,25\\pm\\sqrt{\\frac{25}{16}}`} />
                <BlockMath math={`x_{1,2}=-0,25\\pm\\frac{5}{4}`} />
                <BlockMath math={`x_1=-1,5,\\quad x_2=1`} />
                <p>Berechne die y-Werte mithilfe der Geradengleichung:</p>
                <BlockMath math={`y_1=1,5+2=3,5,\\quad y_2=-1+2=1`} />
                <BlockMath
                  math={`\\Rightarrow\\; S_1(-1,5\\mid 3,5),\\; S_2(1\\mid 1)`}
                />
              </>
            )}
            {data.h != 0 && (
